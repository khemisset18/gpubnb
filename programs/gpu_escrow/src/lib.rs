...
const REFUND_GRACE_SECONDS: i64 = 3_600;
const MAX_BOOKING_SECONDS: u32 = 86_400;
// The API accepts starts up to 30 days ahead and bookings up to 24 hours long.
// This bound accommodates that window plus the mandatory one-hour refund grace.
...
pub mod gpu_escrow {
    ...
    pub fn open(
        ...
        booking_starts_at: i64,
        booking_ends_at: i64,
    ) -> Result<()> {
        let now = Clock::get()?.unix_timestamp;
        let expires_at = refund_deadline(booking_ends_at)?;
        ...
        require!(
            now < booking_starts_at && booking_starts_at < booking_ends_at,
            ErrorCode::InvalidBookingPeriod
        );
        require!(
            expires_at > now
...
fn refund_deadline(booking_ends_at: i64) -> Result<i64> {
    booking_ends_at
        .checked_add(REFUND_GRACE_SECONDS)
        .ok_or_else(|| error!(ErrorCode::Math))
}
...
pub enum ErrorCode {
    ...
    #[msg("invalid expiry")]
    InvalidExpiry,
    #[msg("booking period is invalid or its deposit window has closed")]
...
mod tests {
    ...
    #[test]
    fn commission_is_exactly_five_percent_rounded_down() {
        for amount in [1_u64, 19, 20, 21, 1_000_000_019, u64::MAX] {
            let (payable, fee, provider, refund) = settlement_amounts(amount, 1000, 1000).unwrap();
            assert_eq!(fee as u128, payable as u128 * 500 / 10_000);
            assert_eq!(
                provider as u128 + fee as u128 + refund as u128,
                amount as u128
            );
        }
    }
    #[test]
    fn refund_deadline_is_exactly_one_hour_after_booking_end() {
        assert_eq!(refund_deadline(1_784_723_696).unwrap(), 1_784_727_296);
