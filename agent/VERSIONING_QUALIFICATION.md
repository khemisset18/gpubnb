...
- `0.6.5`: adds fencing-safe recovery of stale local `QUARANTINED` rental claims left by older sessions. Recovery requires a strictly newer server fence, the same physical GPU, no persisted process identity and a fresh quiescence proof; ambiguous state remains fail-closed. This is the preferred Windows Host build for the next physical rental qualification.
