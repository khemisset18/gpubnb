...
def test_stun_parses_valid_ipv4_and_ipv6() -> None:
    transaction = bytes(range(12))
    assert parse_stun_binding_response(stun_response(transaction, "198.51.100.8", 4242), transaction) == (
        "198.51.100.8", 4242
    )
    assert parse_stun_binding_response(stun_response(transaction, "2001:db8::8", 65535), transaction) == (
        "2001:db8::8", 65535
    )
...
def test_stun_rejects_malformed_packets(mutate, code: str) -> None:
    transaction = b"t" * 12
    with pytest.raises(P2PError, match=code):
        parse_stun_binding_response(mutate(stun_response(transaction, "198.51.100.8", 4242)), transaction)


def test_stun_rejects_wrong_transaction_id() -> None:
    packet = stun_response(b"a" * 12, "198.51.100.8", 4242)
    with pytest.raises(P2PError, match="p2p_stun_transaction_mismatch"):
...
def test_discovery_keeps_reserved_port_filters_addresses_and_deduplicates() -> None:
    transaction = b"x" * 12
    fake = FakeSocket(stun_response(transaction, "198.51.100.9", 51000))
    result = discover_candidates(
        {"stunServers": [{"host": "stun.internal", "port": 3478}], "stunTimeoutMs": 50, "stunTotalTimeoutMs": 100},
        address_provider=lambda: map(ipaddress.ip_address, ["127.0.0.1", "0.0.0.0", "169.254.2.2", "10.0.0.4", "10.0.0.4"]),
...
def test_discovery_has_no_default_public_stun_and_enforces_limits() -> None:
    fake = FakeSocket(b"")
    result = discover_candidates(
        {}, address_provider=lambda: [ipaddress.ip_address("10.0.0.2")], socket_factory=lambda *_args: fake
    )
    assert result.candidates == (Candidate("HOST", "10.0.0.2:41000", 100),)
    result.close()

    with pytest.raises(P2PError, match="p2p_stun_config_invalid"):
...
def test_discovery_advertises_ipv6_on_dual_stack_reserved_port() -> None:
    fake = FakeSocket(b"")
    result = discover_candidates(
        {},
    ...
    assert result.candidates == (Candidate("HOST", "[2001:db8::9]:41000", 100),)
    result.close()
...
def signed_ticket(value: dict | None = None, key: SigningKey | None = None) -> tuple[dict, SigningKey]:
    ...
    host = tuple(Candidate(**{"kind": item["kind"], "endpoint": item["endpoint"], "priority": item["priority"]}) for item in ticket_claims["hostCandidates"])
    renter = tuple(Candidate(**{"kind": item["kind"], "endpoint": item["endpoint"], "priority": item["priority"]}) for item in ticket_claims["renterCandidates"])
...
def verify(ticket: dict, key: SigningKey, **overrides):
    values = dict(
        session_id="session_00000001", machine_id="machine_00000001", lease_id="lease_00000001",
        fencing_token="42", now_ms=1_000_010,
    )
    values.update(overrides)
    return verify_rendezvous_ticket(ticket, bytes(key.verify_key), **values)


def test_ticket_signature_bytes_match_rust_contract_and_direct_first_order() -> None:
    ...
    host = tuple(Candidate(item["kind"], item["endpoint"], item["priority"]) for item in claims()["hostCandidates"])
    renter = tuple(Candidate(item["kind"], item["endpoint"], item["priority"]) for item in claims()["renterCandidates"])
    assert _signing_bytes(claims(), host, renter) == expected
    assert [item.kind for item in verify(ticket, key)] == ["HOST", "SERVER_REFLEXIVE", "RELAY"]


def test_ticket_rejects_expiry_bad_signature_and_fencing_mismatch() -> None:
    ticket, key = signed_ticket()
    with pytest.raises(P2PError, match="p2p_ticket_expired"):
        verify(ticket, key, now_ms=1_060_000)
    with pytest.raises(P2PError, match="p2p_ticket_fencing_token_mismatch"):
        verify(ticket, key, fencing_token="43")
    with pytest.raises(P2PError, match="p2p_ticket_signature_invalid"):
        verify(ticket, SigningKey.generate())


@pytest.mark.parametrize(
    ("claim_field", "claim_value", "argument", "argument_value", "code"),
    [
...
def test_ticket_rejects_protocol_and_authority_scope(
    claim_field: str, claim_value, argument: str | None, argument_value, code: str
) -> None:
    value = claims()
...
def test_ticket_rejects_overlong_lifetime() -> None:
    value = claims()
    value["expiresAtMs"] = value["issuedAtMs"] + 120_001
    ticket, key = signed_ticket(value)
    with pytest.raises(P2PError, match="p2p_ticket_time_invalid"):
        verify(ticket, key)


def test_ticket_rejects_duplicate_candidates_relay_only_and_limits() -> None:
    duplicate = claims()
    duplicate["hostCandidates"].append(dict(duplicate["hostCandidates"][0]))
    ticket, key = signed_ticket(duplicate)
    with pytest.raises(P2PError, match="p2p_ticket_host_candidates_duplicate"):
        verify(ticket, key)

    relay_only = claims()
    relay_only["hostCandidates"] = [relay_only["hostCandidates"][0]]
    ticket, key = signed_ticket(relay_only)
    with pytest.raises(P2PError, match="p2p_ticket_relay_only"):
        verify(ticket, key)
    ...
    with pytest.raises(P2PError, match="p2p_ticket_renter_candidates_invalid"):
        verify(ticket, key)


def test_ticket_rejects_relay_under_direct_only_and_unknown_fields() -> None:
    direct_only = claims()
    direct_only["relayPolicy"] = "DIRECT_ONLY"
    ticket, key = signed_ticket(direct_only)
    with pytest.raises(P2PError, match="p2p_ticket_relay_forbidden"):
        verify(ticket, key)

    ticket, key = signed_ticket()
    ticket["claims"]["unexpected"] = "value"
    with pytest.raises(P2PError, match="p2p_ticket_claims_invalid"):
        verify(ticket, key)


def test_errors_are_stable_codes_without_address_values() -> None:
    packet = stun_response(b"a" * 12, "198.51.100.222", 4242)
    with pytest.raises(P2PError) as error:
        parse_stun_binding_response(packet, b"b" * 12)
    assert str(error.value) == "p2p_stun_transaction_mismatch"
    assert "198.51.100.222" not in str(error.value)
