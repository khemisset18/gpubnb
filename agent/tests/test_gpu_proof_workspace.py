...
class GpuProofCommandTests(unittest.TestCase):
    def test_command_is_bounded_and_has_no_host_or_network_access(self):
        command = gpu_proof_command(OFFICIAL_PROOF_IMAGE, 300, "gpubnb-proof-test", GPU_UUID)
    ...
    def test_command_attaches_the_exact_leased_gpu_by_hardware_uuid(self):
        # Regression: the container must never fall back to a fixed Docker device
        # index. It must attach exactly the hardware UUID the rental resource
        # authority leased for this session - otherwise a multi-GPU host could run
        # a renter's paid workload on the wrong physical GPU.
        command = gpu_proof_command(OFFICIAL_PROOF_IMAGE, 60, "gpubnb-proof-test", GPU_UUID)
    ...
    def test_rejects_missing_or_malformed_target_gpu(self):
        for invalid in ("", "short", "device=0; rm -rf /", "0"):
            with self.assertRaisesRegex(RuntimeError, "gpu_proof_invalid_target_gpu"):
                gpu_proof_command(OFFICIAL_PROOF_IMAGE, 60, "gpubnb-proof-test", invalid)
    ...
    def test_gpu_proof_allows_long_uncached_image_pull(self, pull, _popen, _cleanup):
        ...
        with self.assertRaisesRegex(RuntimeError, "stop_after_pull"):
            run_gpu_proof_workspace(OFFICIAL_PROOF_IMAGE, 30, GPU_UUID)
    ...
    def test_duration_is_clamped_and_untrusted_images_are_rejected(self):
        command = gpu_proof_command(OFFICIAL_PROOF_IMAGE, 9999, "gpubnb-proof-test", GPU_UUID)
        self.assertEqual(command[-1], "600")
        with self.assertRaisesRegex(RuntimeError, "gpu_proof_image_not_official_or_pinned"):
            gpu_proof_command("evil.example/workspace@sha256:" + ("b" * 64), 60, "safe", GPU_UUID)
