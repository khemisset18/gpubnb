...
### 2.2 Constat le plus important de cet audit

**`confirmSettlement` (`settlement-transactions.ts`) NE VÉRIFIE PAS on-chain la signature qui lui est fournie.** Contrairement à `verifyOpenEscrowTransaction` (dépôt), qui relit le compte Anchor et compare byte à byte, `confirmSettlement` se contente de valider le **format** de la chaîne (`SIGNATURE_PATTERN`, regex base58) puis l'enregistre comme preuve de règlement. Le contrat Anchor (`programs/gpu_escrow/src/lib.rs`) expose pourtant de vraies instructions de règlement on-chain (`propose_settlement`, `dispute`, `finalize`, `resolve_dispute`, `refund_expired`) — mais **rien dans `apps/api` n'appelle ni ne vérifie ces instructions**. Cette route est protégée par un jeton interne (donc pas exploitable par un utilisateur externe), mais elle repose entièrement sur la confiance envers l'appelant interne, sans preuve cryptographique indépendante que les fonds ont réellement quitté l'escrow. C'est une lacune réelle, distincte de tout ce qui était documenté dans RC1 — elle n'apparaissait pas dans `RISKS_RC1.md` car le chemin de règlement n'avait jamais été audité aussi précisément.

**Ceci n'est pas un défaut introduit par cette branche : le code n'a pas été modifié ici.** C'est une découverte de l'audit, à traiter avant tout déploiement avec de l'argent réel (Devnet inclus, si des locataires bêta doivent voir un vrai remboursement/paiement).
...
3. Combler la lacune de la section 2.2 : soit vérifier on-chain la signature de règlement (comme `verifyOpenEscrowTransaction` le fait pour le dépôt), soit documenter explicitement — et faire accepter par un humain — que le règlement reste une opération de confiance interne pour la durée de la bêta privée.
4. Exécuter un cycle complet réel sur Devnet : dépôt signé par un vrai wallet Phantom Devnet, job réel, règlement, remboursement partiel et total — avec preuve (signatures, montants observés).
