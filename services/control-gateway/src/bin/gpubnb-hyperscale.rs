...
use gpubnb_control_gateway::{
    protocol::{AgentMessage, ClientHello, FenceReason, GatewayMessage},
    qualification::{
        run_capacity_model, summarize_latencies, CapacityProfile, LatencySummary,
        QualificationCheck,
    },
...
const LIVE_MAX_CONNECTIONS_PER_INJECTOR: u64 = 200_000;
const LIVE_FRAME_BYTES: usize = 64 * 1024;
...
impl LiveScenario {
    fn parse(value: &str) -> Result<Self> {
        match value {
            ...
            "replacement-fence" => Ok(Self::ReplacementFence),
            _ => bail!("scenario must be steady, reconnect-storm, or replacement-fence"),
        }
    }
    ...
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ReplacementFence => "replacement-fence",
        }
    }
}
...
struct LiveAccumulator {
    ...
    expected_fences: std::sync::atomic::AtomicU64,
    observed_fences: std::sync::atomic::AtomicU64,
}
...
struct LiveReport {
    ...
    heartbeat_attempts: u64,
    heartbeat_failures: u64,
    presence_probe_attempts: u64,
    presence_probe_failures: u64,
    expected_fences: u64,
    observed_fences: u64,
    connection_failure_bps: u64,
    heartbeat_failure_bps: u64,
    presence_probe_failure_bps: u64,
    connect_latency: LatencySummary,
    presence_commit_latency: LatencySummary,
...
async fn main() {
    if let Err(error) = run().await {
        eprintln!("gpubnb-hyperscale: {error:#}");
        std::process::exit(2);
    }
}
...
async fn run() -> Result<()> {
    ...
    match command {
        "model" => run_model_command(&args),
        "live" => run_live_command(&args).await,
        "help" | "--help" | "-h" => {
            print_usage();
            Ok(())
        }
...
fn run_model_command(args: &ParsedArgs) -> Result<()> {
    let profile_path = PathBuf::from(args.required("--profile")?);
    let profile_raw = std::fs::read(&profile_path)
        .with_context(|| format!("failed to read capacity profile {}", profile_path.display()))?;
    let profile: CapacityProfile =
        serde_json::from_slice(&profile_raw).context("capacity profile JSON invalid")?;
    let report = run_capacity_model(&profile)?;
    let encoded = serde_json::to_string_pretty(&report)?;
    write_output(args.value("--output"), &encoded)?;
    println!("{encoded}");
    if args.flag("--require-pass") && !report.passed {
        bail!("capacity model failed one or more qualification gates");
    }
    Ok(())
}

async fn run_live_command(args: &ParsedArgs) -> Result<()> {
    if !args.flag("--allow-test-auth-write") {
        bail!("live mode requires --allow-test-auth-write because it seeds isolated load-test machine keys in Redis");
    }
    let config = LiveConfig {
        scenario: LiveScenario::parse(args.value("--scenario").unwrap_or("steady"))?,
        gateway: args
            .required("--gateway")?
            .parse()
        ...
        connections: parse_u64(
            ...
            128,
            1,
            LIVE_MAX_CONNECTIONS_PER_INJECTOR,
            "--connections",
        )?,
    ...
    let report = run_live(config.clone()).await?;
    let encoded = serde_json::to_string_pretty(&report)?;
    write_output(args.value("--output"), &encoded)?;
    println!("{encoded}");
    if config.require_pass && !report.passed {
        bail!("live qualification failed one or more gates");
    }
    Ok(())
}

async fn run_live(config: LiveConfig) -> Result<LiveReport> {
    ...
    let redis = redis_client
        ...
        .await
        .context("failed to connect load injector to Redis")?;
    seed_test_auth(&redis, config.connections, config.seed).await?;

    let accumulator = Arc::new(LiveAccumulator::default());
    ...
    for index in 0..config.connections {
        ...
        tasks.push(tokio::spawn(async move {
            barrier.wait().await;
            if config.ramp_ms > 0 {
                let delay = deterministic_delay_ms(config.seed, index, 0, config.ramp_ms);
                tokio::time::sleep(Duration::from_millis(delay)).await;
            }
            match config.scenario {
                LiveScenario::Steady => {
                    run_steady_agent(index, &config, &endpoint, redis, &accumulator).await
                }
                LiveScenario::ReconnectStorm => {
                    run_reconnect_agent(index, &config, &endpoint, &accumulator).await
                }
                LiveScenario::ReplacementFence => {
                    run_replacement_fence_agent(index, &config, &endpoint, &accumulator).await
                }
            }
        }));
    }
    ...
    endpoint.close(0_u8.into(), b"qualification complete");
    endpoint.wait_idle().await;

    if !config.keep_test_auth {
        cleanup_test_auth(&redis, config.connections).await?;
    }
    ...
    let presence_probe_failures = accumulator
        ...
        .load(std::sync::atomic::Ordering::Relaxed);
    let expected_fences = accumulator
        .expected_fences
        .load(std::sync::atomic::Ordering::Relaxed);
    let observed_fences = accumulator
        .observed_fences
        .load(std::sync::atomic::Ordering::Relaxed);
    let mut connect_latencies = accumulator
        .connect_latencies_ms
        .lock()
        .expect("latency mutex poisoned")
        .clone();
    let mut presence_latencies = accumulator
        .presence_latencies_ms
        .lock()
        .expect("latency mutex poisoned")
        .clone();
    let connect_latency = summarize_latencies(&mut connect_latencies);
    let presence_commit_latency = summarize_latencies(&mut presence_latencies);
    let connection_attempts = successful_connections.saturating_add(failed_connections);
    let connection_failure_bps = failure_bps(failed_connections, connection_attempts);
    let heartbeat_failure_bps = failure_bps(heartbeat_failures, heartbeat_attempts);
    let presence_probe_failure_bps = failure_bps(presence_probe_failures, presence_probe_attempts);

    let mut checks = vec![
        live_check(
            "connection_failure_rate",
            connection_failure_bps,
            config.max_failure_bps,
            "basis_points",
        ),
        live_check(
        ...
        live_check(
            "presence_probe_failure_rate",
            presence_probe_failure_bps,
            config.max_failure_bps,
            "basis_points",
        ),
        live_check(
            "connect_p99_latency",
            connect_latency.p99_ms,
            config.max_connect_p99_ms,
            "milliseconds",
        ),
    ];
    if config.scenario == LiveScenario::ReplacementFence {
        checks.push(QualificationCheck {
            name: "replacement_connection_fence_delivery".into(),
            passed: observed_fences == expected_fences,
            observed: observed_fences,
            limit: expected_fences,
            unit: "exact_fences".into(),
        });
    }
...
async fn run_steady_agent(
    ...
    let key = signing_key(config.seed, index);
    let machine_id = machine_id(index);
    ...
    while started.elapsed() < Duration::from_secs(config.duration_seconds) {
        ...
        if heartbeat_sequence == 1 && index % config.presence_sample_every == 0 {
            accumulator
                .presence_probe_attempts
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            match wait_presence_sequence(&mut redis, &machine_id, heartbeat_sequence).await {
                Ok(()) => accumulator
                    .presence_latencies_ms
                    .lock()
                    .expect("latency mutex poisoned")
    ...
    agent
        .connection
        .close(0_u8.into(), b"steady qualification complete");
}
    ...
    for cycle in 0..config.reconnect_cycles {
        ...
        .await
        {
            Ok(mut agent) => {
                ...
                if write_json_frame(&mut agent.send, &heartbeat, LIVE_FRAME_BYTES)
                    .await
                    .is_err()
                {
                    accumulator
                ...
                agent
                    .connection
                    .close(0_u8.into(), b"reconnect cycle complete");
            }
            Err(error) => {
                accumulator
                    .failed_connections
...
async fn run_replacement_fence_agent(
    index: u64,
    config: &LiveConfig,
    endpoint: &quinn::Endpoint,
    accumulator: &LiveAccumulator,
    ...
    let key = signing_key(config.seed, index);
    let machine_id = machine_id(index);
    let mut first = match connect_agent(
        endpoint,
        config.gateway,
        &config.server_name,
        &machine_id,
    ...
    .await
    {
        ...
        Err(error) => {
            accumulator
                .failed_connections
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            accumulator.failure(&format!("first_connect:{}", error_class(&error)));
            return;
        }
    };
    accumulator
    ...
    accumulator
        ...
        .lock()
        .expect("latency mutex poisoned")
        .push(first.connect_latency_ms);
    accumulator
        .expected_fences
        .fetch_add(1, std::sync::atomic::Ordering::Relaxed);

    let second = match connect_agent(
        endpoint,
        config.gateway,
        &config.server_name,
        &machine_id,
    ...
    .await
    {
        ...
        Err(error) => {
            accumulator
                .failed_connections
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            accumulator.failure(&format!("replacement_connect:{}", error_class(&error)));
            first.connection.close(0_u8.into(), b"replacement failed");
            return;
        }
    };
    accumulator
    ...
    accumulator
        ...
        .lock()
        .expect("latency mutex poisoned")
        .push(second.connect_latency_ms);

    match tokio::time::timeout(
        Duration::from_secs(2),
        read_json_frame::<GatewayMessage>(&mut first.recv, LIVE_FRAME_BYTES),
    )
    .await
    {
        Ok(Ok(GatewayMessage::Fence {
        ...
        Ok(Ok(_)) => accumulator.failure("replacement_fence:unexpected_frame"),
        Ok(Err(_)) => accumulator.failure("replacement_fence:read_error"),
        Err(_) => accumulator.failure("replacement_fence:timeout"),
    }
    first
        .connection
        .close(0_u8.into(), b"fenced connection closed");
    second
        .connection
        .close(0_u8.into(), b"replacement qualification complete");
}
...
async fn connect_agent(
    ...
    write_json_frame(&mut send, &hello, LIVE_FRAME_BYTES).await?;
    let frame: GatewayMessage = tokio::time::timeout(
        Duration::from_secs(CONNECT_TIMEOUT_SECONDS),
        read_json_frame(&mut recv, LIVE_FRAME_BYTES),
    )
    .await
...
async fn seed_test_auth(redis: &MultiplexedConnection, connections: u64, seed: u64) -> Result<()> {
    let mut redis = redis.clone();
    for batch_start in (0..connections).step_by(1_000) {
        let batch_end = connections.min(batch_start + 1_000);
        let mut pipeline = redis::pipe();
        for index in batch_start..batch_end {
            let machine = machine_id(index);
            let key = signing_key(seed, index);
        ...
        let _: () = pipeline
            ...
            .await
            .context("failed to seed test auth cache")?;
    }
    Ok(())
}

async fn cleanup_test_auth(redis: &MultiplexedConnection, connections: u64) -> Result<()> {
    let mut redis = redis.clone();
    for batch_start in (0..connections).step_by(1_000) {
        let batch_end = connections.min(batch_start + 1_000);
        let mut pipeline = redis::pipe();
        ...
        let _: () = pipeline
            .query_async(&mut redis)
            .await
            .context("failed to clean test auth cache")?;
    }
    Ok(())
}
...
fn signing_key(seed: u64, index: u64) -> SigningKey {
...
fn machine_id(index: u64) -> String {
    format!("load_machine_{index:012}")
}
...
fn live_check(name: &str, observed: u64, limit: u64, unit: &str) -> QualificationCheck {
    QualificationCheck {
        name: name.to_owned(),
...
fn error_class(error: &anyhow::Error) -> String {
    ...
    if raw.contains("timeout") {
        ...
        "tls".into()
    } else if raw.contains("authentication") || raw.contains("hello") {
        "auth".into()
    } else if raw.contains("redis") {
        "redis".into()
    } else {
        "other".into()
    }
...
fn parse_u64(raw: Option<&str>, default: u64, min: u64, max: u64, name: &str) -> Result<u64> {
    let value = match raw {
        Some(raw) => raw
            .parse::<u64>()
...
fn write_output(path: Option<&str>, encoded: &str) -> Result<()> {
    if let Some(path) = path {
        std::fs::write(path, encoded).with_context(|| format!("failed to write report {path}"))?;
    }
    Ok(())
}
    ...
    fn parse(raw: &[String]) -> Result<Self> {
        ...
        while index < raw.len() {
            ...
            if index + 1 < raw.len() && !raw[index + 1].starts_with("--") {
                if values.insert(key.clone(), raw[index + 1].clone()).is_some()
                    || flags.contains(key)
                {
                ...
                if !flags.insert(key.clone()) || values.contains_key(key) {
...
fn print_usage() {
    eprintln!(
        "Usage:\n  gpubnb-hyperscale model --profile <json> [--output report.json] [--require-pass]\n\n  gpubnb-hyperscale live --gateway 127.0.0.1:4443 --server-name localhost --ca-cert cert.pem \\\n    --redis-url redis://127.0.0.1:6379 --allow-test-auth-write [--scenario steady|reconnect-storm|replacement-fence] \\\n    [--connections 128] [--duration-seconds 5] [--heartbeat-ms 1000] [--ramp-ms 1000] \\\n    [--reconnect-cycles 3] [--reconnect-jitter-ms 1000] [--presence-sample-every 16] \\\n    [--max-connect-p99-ms 5000] [--max-failure-bps 100] [--output report.json] [--require-pass]"
    );
}
