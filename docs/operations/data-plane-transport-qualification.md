...
|---|---:|---|
| Remote bidi streams / connection | 75 | 64 workspace + 1 authority/control + 10 bounded stream-credit replenishment reserve |
...
The 11 non-workspace bidi credits are protocol/transport infrastructure, not additional workspace capacity. One covers the one-shot authority/control stream and ten form a bounded stream-credit replenishment reserve. Quinn batches `MAX_STREAMS` updates until more than one eighth of the advertised remote-stream window has been freed. With 75 advertised bidi streams, that batching threshold is nine streams, so a reserve of ten guarantees bounded progress for explicit over-capacity rejection and normal stream churn until the next cumulative credit update. A compile-time invariant keeps the reserve above the batching threshold. `EdgeRegistry` remains the authoritative application limit at 64 workspace streams.

After successful Host or Renter authentication, the Edge drops the completed control `SendStream` before entering the long-lived connection loop. When all 64 workspace streams are live, an additional workspace attempt can still reach the application layer and receive an explicit `STREAM_LIMIT` instead of becoming an ambiguous QUIC-layer stall.

Routed renter stream capacity is released only after both relay directions are complete and the Edge's response FIN has been acknowledged by the renter (`SendStream::stopped()` completes with `None`). This keeps `EdgeRegistry` capacity aligned with QUIC stream lifecycle. The qualification test then rotates ten workspace streams while maintaining the 64-stream application ceiling, proving that the bounded reserve carries churn until Quinn replenishes cumulative `MAX_STREAMS` credit.
...
3. **Stream pressure:** all 64 workspace application streams can be admitted concurrently; a 65th workspace attempt reaches application admission through the bounded transport reserve and receives `STREAM_LIMIT`; then ten close-and-replacement rotations succeed while application concurrency remains capped at 64, proving cumulative QUIC stream credit is replenished without transport stalls.
