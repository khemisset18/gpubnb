...
check_not_grep "Anchor.toml has no placeholder" 'REPLACE_WITH_DEPLOYED_PROGRAM_ID' programs/gpu_escrow/Anchor.toml
if find . -path './apps/api/node_modules' -prune -o -name '*keypair*.json' -print | grep -q .; then echo 'FAIL: no deploy key committed'; failed=1; else echo 'OK:   no deploy key committed'; fi
