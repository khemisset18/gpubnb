...
const renderPairing = (status: HostStatus): string => {
  ...
  return `<section class="explanation pairing-guide"><p class="eyebrow">Connexion sécurisée</p><h2>Reliez cet ordinateur à votre compte</h2>
    ...
    ${status.pairing.configured ? `<form id="pairing-form" class="pairing-form" novalidate><label for="pairing-code">Code de liaison</label><div class="pairing-controls">
      ...
      <button id="pairing-submit" class="primary large" type="submit">Relier cette machine</button></div><small>Le code est transmis directement à l’agent local.</small></form>` : ''}
...
const renderGpuInventory = (status: HostStatus): string => {
  ...
  if (gpus.length === 0) return `<section class="gpu-inventory"><p class="eyebrow">Matériel détecté</p><h2>Aucun GPU NVIDIA détecté</h2><p>Branchez un GPU compatible puis relancez la vérification.</p></section>`;
...
const listingUrl = (baseUrl: string, machineId: string, gpuUuid: string): string => {
...
const bindActions = (status: HostStatus): void => {
  ...
  document.querySelectorAll<HTMLButtonElement>('.create-listing').forEach((button) => button.addEventListener('click', () => {
    ...
    setMessage(opened ? 'Annonce préremplie ouverte sur GPUbnb.' : `Ouvrez cette adresse : ${target}`, opened ? 'success' : 'error');
  }));
  document.querySelectorAll<HTMLButtonElement>('[data-action]').forEach((button) => button.addEventListener('click', () => {
    const actionId = button.dataset.action;
    if (!actionId) return;
    if (actionId === 'account' && status.pairing.browserUrl) { const opened = window.open(status.pairing.browserUrl, '_blank', 'noopener,noreferrer'); setMessage(opened ? 'Le site GPUbnb a été ouvert.' : `Ouvrez cette adresse : ${status.pairing.browserUrl}`, opened ? 'success' : 'error'); return; }
