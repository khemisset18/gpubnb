...
function profileOptions(resource){const vendor=vendorName(resource.gpuVendor);const profiles=miningCatalog.profiles.filter(profile=>profile.resourceKind===resource.kind&&(resource.kind==='CPU'||profile.gpuVendors.includes(vendor)));return profiles.map(profile=>{const label=`${profile.cryptocurrency} · ${profile.algorithm} · ${profile.miner}`;return `<option value="${escapeHTML(profile.id)}"${profile.id===resource.profileId?' selected':''}>${escapeHTML(label)}</option>`}).join('')}
...
const MINING_TELEMETRY_FRESH_MS=120000;
function metric(value,suffix=''){return typeof value==='number'&&Number.isFinite(value)?`${escapeHTML(Math.round(value*100)/100)}${suffix}`:'—'}
function telemetryAge(lastTelemetryAt){
  const parsed=Date.parse(lastTelemetryAt||'');
  if(!Number.isFinite(parsed))return null;
  return Math.max(0,Date.now()-parsed);
}
function renderLiveTelemetry(resource){
  if(resource.kind!=='GPU')return '';
  const age=telemetryAge(resource.lastTelemetryAt);
  const telemetry=resource.lastTelemetry;
  if(age===null||!telemetry||typeof telemetry!=='object'){
    return '<section class="mining-live-telemetry"><p class="muted">Télémétrie Agent : aucune mesure reçue.</p></section>';
  }
  if(age>MINING_TELEMETRY_FRESH_MS){
    const seconds=Math.round(age/1000);
    return `<section class="mining-live-telemetry"><p class="muted">Télémétrie Agent périmée depuis ${escapeHTML(seconds)} s. Les anciennes métriques ne sont pas affichées comme valeurs actuelles.</p></section>`;
  }
  const shares=[telemetry.acceptedShares,telemetry.staleShares,telemetry.hardwareErrors].map(value=>Number.isInteger(value)?value:'—').join(' / ');
  return `<section class="mining-live-telemetry"><div class="actions" style="gap:12px;flex-wrap:wrap"><span class="badge ok">Télémétrie fraîche</span><span class="muted">mise à jour il y a ${escapeHTML(Math.round(age/1000))} s</span></div><dl class="performance-grid"><div><dt>Hashrate</dt><dd>${metric(telemetry.hashrate,telemetry.hashrateUnit?` ${telemetry.hashrateUnit}`:'')}</dd></div><div><dt>Température</dt><dd>${metric(telemetry.temperatureC,' °C')}</dd></div><div><dt>Puissance</dt><dd>${metric(telemetry.powerWatts,' W')}</dd></div><div><dt>Utilisation GPU</dt><dd>${metric(telemetry.utilizationPercent,' %')}</dd></div><div><dt>Parts A/S/Hw</dt><dd>${escapeHTML(shares)}</dd></div><div><dt>Pool</dt><dd>${telemetry.poolConnected?'Connecté':'Non confirmé'}</dd></div><div><dt>Uptime</dt><dd>${Number.isInteger(telemetry.uptimeSeconds)?`${escapeHTML(telemetry.uptimeSeconds)} s`:'—'}</dd></div></dl></section>`;
}
...
function renderResource(resource){
  ...
  const thermalPolicy=miningCatalog.thermalLimits[resource.kind]||{minimumC:85,maximumC:98};
  const thermalMinimum=thermalPolicy.minimumC;
  const thermalMaximum=thermalPolicy.maximumC;
  const thermalValue=Number.isInteger(resource.maximumTemperatureC)?resource.maximumTemperatureC:thermalMinimum;
