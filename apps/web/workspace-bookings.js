...
(() => {
  ...
  function mobileBlockHTML(booking,job,detail,errorMessage){
    const phase=deriveDeveloperPhase({bookingStatus:booking.status,gpuProofJob:job,workspaceDetail:detail});
    if(phase===DeveloperPhase.HIDDEN)return '';
    const title=escapeHTML(booking.listing?.title||'Réservation GPU');
    const errorHTML=errorMessage?`<div class="muted">${escapeHTML(errorMessage)}</div>`:'';
    let action='';let badge=`<span class="badge">${escapeHTML(detail?.status||'')}</span>`;
    if(phase===DeveloperPhase.CREATE){
      action=`<button class="button button-primary" type="button" data-create-mobile="${escapeHTML(booking.id)}">Créer mon espace Mobile</button>`;
      badge='';
    }else if(phase===DeveloperPhase.PREPARING){
      action=`<span class="muted">${escapeHTML(preparationLabel(detail))}</span>`;
      badge='';
    }else if(phase===DeveloperPhase.OPEN){
      // Real Android SDK/Gradle build environment in the same code-server
      // terminal/editor as Developer Workspace - no graphical emulator (see
      // workspace-manifests.ts and workspaces/mobile/welcome.md).
      action=`<button class="button button-primary" type="button" data-open-mobile="${escapeHTML(booking.id)}">Ouvrir (Android SDK/Gradle, sans émulateur)</button>`;
      badge='<span class="badge ok">PRÊT</span>';
    }else if(phase===DeveloperPhase.RETRY){
      action=`<button class="button" type="button" data-retry-mobile="${escapeHTML(booking.id)}">Réessayer</button>`;
      badge=`<span class="badge warn">${escapeHTML(detail?.preparation?.errorCode||detail?.status||'ÉCHEC')}</span>`;
    }else if(phase===DeveloperPhase.ENDED){
      badge=`<span class="badge">${escapeHTML(detail?.status||'TERMINÉ')}</span>`;
    }
  ...
  async function render(){
    ...
    try{
      ...
      await Promise.all(eligible.filter(row=>!mobileActionInFlight.has(row.booking.id)).map(async row=>{
        try{
          const detail=await request(`/bookings/${encodeURIComponent(row.booking.id)}/workspace/mobile/status`);
          mobileDetailByBooking.set(row.booking.id,detail);
        }catch(error){
          if(error.status===404){mobileDetailByBooking.set(row.booking.id,null);}
        }
      }));

      const mobileHTML=eligible.map(row=>mobileBlockHTML(
      ...
      async function runMobileAction(bookingId,button,busyText,run){
        if(mobileActionInFlight.has(bookingId))return;
        mobileActionInFlight.add(bookingId);
        button.disabled=true;const originalText=button.textContent;button.textContent=busyText;
        mobileErrors.delete(bookingId);
        try{
          await run();
          mobileActionInFlight.delete(bookingId);
          await render();
        }catch(error){
          mobileActionInFlight.delete(bookingId);
          mobileErrors.set(bookingId,error.message||'Action impossible.');
          button.disabled=false;button.textContent=originalText;
          await render();
        }
      }

      root.querySelectorAll('[data-create-mobile]').forEach(button=>button.addEventListener('click',()=>{
        const bookingId=button.dataset.createMobile;
        runMobileAction(bookingId,button,'Création…',()=>
          request(`/bookings/${encodeURIComponent(bookingId)}/workspace/mobile`,{method:'POST'}),
        );
      }));

      root.querySelectorAll('[data-retry-mobile]').forEach(button=>button.addEventListener('click',()=>{
        const bookingId=button.dataset.retryMobile;
        runMobileAction(bookingId,button,'Nouvelle tentative…',()=>
          request(`/bookings/${encodeURIComponent(bookingId)}/workspace/retry`,{method:'POST'}),
        );
      }));

      root.querySelectorAll('[data-open-mobile]').forEach(button=>button.addEventListener('click',()=>{
