...
test('rental cleanup never schedules mining auto-resume when mining rollout is disabled', async () => {
  ...
  assert.match(body, /miningCommandGatewayAssigned\(machineId, commandDispatchConfigFromEnv\(\)\)/);
  ...
  assert.ok(
    ...
    'mining rollout gate must run before auto-resume can create a durable START command',
  );
});
