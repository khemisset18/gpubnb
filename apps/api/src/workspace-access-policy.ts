...
const ACTIVE_BOOKINGS=new Set(['FUNDED','STARTING','RUNNING','ACTIVE']);
const OPENABLE_SESSIONS=new Set(['READY','ACTIVE']);
...
export function evaluateWorkspaceAccess(input:WorkspaceAccessPolicyInput):WorkspaceAccessPolicyResult{
  ...
  if(input.machineOperational==='QUARANTINED'||input.moderationStatus!=='CLEAR')return {allowed:false,code:'MACHINE_BLOCKED'};
