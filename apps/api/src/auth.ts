...
import { validCsrfToken } from './security.js';
export type Session={userId:string,wallet:string,sessionVersion:number,createdAt:string,lastSeenAt:string,csrfToken:string};
...
export async function issueSession(redis:Redis,reply:FastifyReply,session:Omit<Session,'lastSeenAt'|'csrfToken'>){
 const token=crypto.randomBytes(32).toString('base64url'); const csrfToken=crypto.randomBytes(32).toString('base64url'); const now=new Date().toISOString();
 await redis.set(`session:${tokenHash(token)}`,JSON.stringify({...session,lastSeenAt:now,csrfToken}),'EX',config.SESSION_TTL_SECONDS);
...
export async function requireSession(req:FastifyRequest,reply:FastifyReply,redis:Redis):Promise<Session|undefined>{
 ...
 if(typeof session.csrfToken!=='string'||session.csrfToken.length<32){await redis.del(key);reply.code(401).send({error:'session_invalid'});return}
 if(['POST','PUT','PATCH','DELETE'].includes(req.method)){
  const header=Array.isArray(req.headers['x-csrf-token'])?req.headers['x-csrf-token'][0]:req.headers['x-csrf-token'];
  if(!validCsrfToken(header,session.csrfToken)){reply.code(403).send({error:'csrf_token_invalid'});return}
 }
