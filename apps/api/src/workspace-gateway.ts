...
export function registerWorkspaceGatewayRoutes(app:FastifyInstance,db:PrismaClient,redis:Redis):void{
  ...
  app.post('/agent/workspace-gateway/:sessionId/stopped',async(request,reply)=>{
    ...
    const row=await db.workspaceSession.findFirst({where:{id:sessionId,machineId,status:{in:[WorkspaceSessionStatus.STOP_REQUESTED,WorkspaceSessionStatus.STOPPING,WorkspaceSessionStatus.RUNNING,WorkspaceSessionStatus.READY]}},select:{id:true}});if(!row)return reply.code(409).send({error:'workspace_not_stoppable'});
    await db.$transaction([db.workspaceSession.update({where:{id:row.id},data:{status:WorkspaceSessionStatus.COMPLETED,endedAt:new Date(),connectionMetadata:{},events:{create:{actorType:'AGENT',actorId:machineId,action:'GATEWAY_CLEANUP_VERIFIED'}}}}),db.machine.update({where:{id:machineId},data:{operational:MachineOperational.AVAILABLE}})]);return {ok:true};
  });
  ...
  app.server.on('upgrade',async(request,socket,head)=>{try{const url=new URL(request.url||'/',`https://${request.headers.host||'localhost'}`);const match=url.pathname.match(/^\/workspace-gateway\/([^/]+)\/(.*)$/);if(!match)return;const sessionId=match[1];const token=parseCookie(request.headers.cookie,GATEWAY_COOKIE);if(!token){socket.destroy();return;}const raw=await redis.get(gatewaySessionKey(token));if(!raw){socket.destroy();return;}const browser=JSON.parse(raw) as {userId:string;sessionId:string};if(browser.sessionId!==sessionId){socket.destroy();return;}const row=await activeGatewaySession(db,sessionId);if(!row||row.renterId!==browser.userId){socket.destroy();return;}
    wss.handleUpgrade(request,socket as import('node:net').Socket,head,(ws:WebSocket)=>{const channelId=crypto.randomUUID();const targetPath='/'+match[2]+url.search;void redis.lpush(machineQueue(row.machineId),JSON.stringify({id:crypto.randomUUID(),sessionId,kind:'ws_open',channelId,path:targetPath,headers:relayHeaders(request.headers as Record<string,unknown>)} satisfies RelayRequest));const pump=setInterval(async()=>{for(let i=0;i<50;i++){const frame=await redis.rpop(wsInputKey(channelId));if(!frame)break;const parsed=JSON.parse(frame) as {dataBase64:string;binary:boolean;close:boolean};if(parsed.close){ws.close();break;}if(ws.readyState===WebSocket.OPEN)ws.send(Buffer.from(parsed.dataBase64,'base64'),{binary:parsed.binary});}},20);ws.on('message',(data:WebSocket.Data,isBinary:boolean)=>void redis.lpush(machineQueue(row.machineId),JSON.stringify({id:crypto.randomUUID(),sessionId,kind:'ws_send',channelId,dataBase64:Buffer.from(data as Buffer).toString('base64'),binary:isBinary} satisfies RelayRequest)));ws.on('close',()=>{clearInterval(pump);void redis.lpush(machineQueue(row.machineId),JSON.stringify({id:crypto.randomUUID(),sessionId,kind:'ws_close',channelId} satisfies RelayRequest));});});
