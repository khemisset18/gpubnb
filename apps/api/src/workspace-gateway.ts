  ...
  app.server.on('upgrade',async(request,socket,head)=>{
    ...
    try{
      ...
      wss.handleUpgrade(request,socket as Socket,head,(ws:WebSocket)=>{
        ...
        ws.on('close',()=>{clearInterval(pump);app.log.info({event:'workspace_gateway_browser_closed',sessionId,machineId:row.machineId,channel:channelLogId},'workspace gateway browser websocket closed');void setup.catch(()=>undefined).then(async()=>{await redis.lpush(machineQueue(row.machineId),JSON.stringify({id:crypto.randomUUID(),sessionId,kind:'ws_close',channelId} satisfies RelayRequest));await redis.del(wsChannelKey(channelId),wsUpstreamReadyKey(channelId));}).catch(error=>app.log.error({err:error,event:'workspace_gateway_close_cleanup_failed',sessionId,machineId:row.machineId,channel:channelLogId},'workspace gateway close cleanup failed'));});
      });
    }catch(error){
